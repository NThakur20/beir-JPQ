...
class TrainQueryDataset(SequenceDataset):
    def __init__(self, queryids_cache, 
            rel_file, max_query_length, hard_negative_jsonl):
        ...
        # with open(os.path.join(hard_negative_jsonl), encoding='utf8') as fIn:
        #     for line in fIn:
        #         line = json.loads(line)
        #         qid = int(line["qid"])
        #         for k, v in line["neg"].items():
        #             for doc_id in v:
        #                 self.negdict[qid].append(int(doc_id))
...
def compute_loss(query_embeddings, pq_codes, centroids, 
    ...
    for qid, qembedding, retrieve_pids, cur_rel_pids, cur_neg_ids in zip(
        ...
        # retrieve_pids = torch.cat((retrieve_pids, torch.tensor(cur_neg_ids).to(batch_neighbors.device)), 0)
...
def train(args, model, pq_codes, centroid_embeds, opq_transform, opq_index):
    ...
    cross_encoder = CrossEncoder('cross-encoder/ms-marco-MiniLM-L-6-v2', device="cuda", max_length=350)
...
def run_parse_args():
    ...
    parser.add_argument("--hard_negative_jsonl", default=None, required=True)
